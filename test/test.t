Simple test with foo
  $ echo "insert foo 42" | MIOU_DOMAINS=1 bancos.sdb -i rowex.00.idx
  db: 1 action(s) committed
  $ echo "find foo" | MIOU_DOMAINS=1 bancos.sdb -i rowex.00.idx 
  "foo" => 42
  db: 1 action(s) committed
Two key with a prefix [e]
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.01.idx <<EOF
...
Three key with a prefix [o]
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.02.idx <<EOF
  > insert desirous@pandering.us 8252
  ...
  > EOF
Little corpus
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.03.idx -c 001.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.03.idx -c 002.cmds
Replacement of <sales@balimandira.com>
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.04.idx -c 007.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.04.idx -c 008.cmds
Rebalance due to different prefix
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.05.idx <<EOF
  > insert dedicated@crocks.net 9554
  ...
  > EOF
Bigger corpus!
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.07.idx -c 003.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.07.idx -c 004.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.08.idx -c 005.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.08.idx -c 006.cmds
Repeat, remove and insert!
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.09.idx -c 005.cmds
  $ MIOU_DOMAINS=1 bancos.sdb -qi rowex.09.idx -c 005.cmds
