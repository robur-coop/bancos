...
int ml_n16_get_child(const uint16_t compact_count, const uint8_t k,
                     const uint8_t keys[16]) {
  __m128i cmp = _mm_cmpeq_epi8(_mm_set1_epi8(k ^ 128),
                               _mm_loadu_si128((const __m128i *)(keys)));
  unsigned bitfield = _mm_movemask_epi8(cmp) & ((1 << compact_count) - 1);
  return (bitfield);
}

CAMLprim value caml_n16_get_child(value compact_count, value k, value keys) {
  return (Val_long(ml_n16_get_child(Long_val(compact_count), Long_val(k),
